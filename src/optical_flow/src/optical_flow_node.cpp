...
Eigen::Vector2f calcTranslationRANSAC(const vector<pair<cv::Point2f, cv::Point2f>>& pairs,
                                      vector<pair<cv::Point2f, cv::Point2f>>& inliers)
{
  const int iter_num = 50, sample_num = 1, total_num = pairs.size();
...
void imageCallback(const sensor_msgs::Image::ConstPtr& image_msg)
{
  ...
  cv::Mat tmp1;
  cv::undistort(tmp, tmp1, cameraMatrix, distCoeffs);
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber sub_image = node.subscribe("/camera/image_raw", 100, imageCallback);
